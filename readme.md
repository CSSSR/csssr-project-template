...
Для создания спрайта нужно добавить папку в `app/sprites` и в неё PNG иконки. Важно, чтобы иконки были с чётными высотой и шириной кратными двум. Retina картинки добавлять в эту папку рядом с обычными иконками и в конце названия файла добавить `@2x`, например:
```
└── app/
    └── sprites/
        └── emoji/
...
* После сборки в папке `app/resources/images/sprites` появятся 2 спрайта (обычный и Retina с `@2x`) и 1 стилевой файл с примесями. Все файлы будут такими же названиями, как у папки, в которой находятся его иконки. Например:
```
└── app/
    ├── resources/
    │   └── images/
