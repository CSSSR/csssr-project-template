...
* После сборки в папке `app/resources/images/sprites` появятся два спрайта: обычный и Retina с `@2x` и в `app/styles/sprites` один стилевой файл с примесями. Все файлы будут с такими же названиями, как у папки, в которой находятся его иконки. Например:
```
└── app/
    ├── resources/
    │   └── images/
...
* При сборке используется `gulp-imagemin`, который слабо жмёт, поэтому желательно после сборке пожать спрайт руками, например, на [tinypng.com](https://tinypng.com/).
